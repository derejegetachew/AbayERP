...
var SearchField = new Ext.form.ComboBox({
	...
	store: new Ext.data.ArrayStore({
		...
		data: [
			['Report.name', '<?php __('Report'); ?>']
,			['GeneretReport.name', '<?php __('Name'); ?>']
,			['GeneretReport.type_ofreport', '<?php __('Type Ofreport'); ?>']
,			['GeneretReport.date', '<?php __('Date'); ?>']
,			['GeneretReport.no', '<?php __('No'); ?>']
		]
	}),
	mode: 'local',
...
var generetReportSearchForm = new Ext.form.FormPanel({
	baseCls: 'x-plain',
	labelWidth: 55,
		...
		
		{
			...
			handler: function(){
				if(ParamStore.getCount() == 0)
					...
					generetReportSearchWindow.close();
...
var generetReportSearchWindow = new Ext.Window({
	title: '<?php __('Search GeneretReport'); ?>',
	modal: true,
	items: generetReportSearchForm,
	resizable: false,
	width: 550,
	height: 400,
